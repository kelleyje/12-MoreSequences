...
def run_test_make_less_simple_string():
    ...
    # Test 2:
    expected = '14-15-16'
    actual = make_less_simple_string(14, 16)
    print('Expected:', expected)
    print('Actual:  ', actual)
...
def rectangles_from_circles(circles):
    ...
    for k in circles:
        center = k.center
        top_left = rg.Point(center.x - k.radius, center.y - k.radius)
        bottom_right = rg.Point(center.x + k.radius, center.y + k.radius)
        note = rg.Rectangle(top_left, bottom_right)
        rects += [note]
